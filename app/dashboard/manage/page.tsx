...
export default function CustomerManagementPage() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      // Fetch decisions for rejected applications
      const rejectedCustomers = data.filter((c: Customer) => c.applicationStatus === "REJECTED");
      for (const customer of rejectedCustomers) {
    ...
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium block">Loan Type Filter</label>
            <select 
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="p-3 text-left text-sm font-semibold">Loan Details</th>
                <th className="p-3 text-left text-sm font-semibold">Status</th>
                <th className="p-3 text-left text-sm font-semibold">Date Applied</th>
              ...
              {filteredCustomers.map((customer) => (
                <tr key={customer.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3">
                    ...
                    <div className="text-sm">{formatCurrency(customer.loanAmount)}</div>
                  </td>
                  <td className="p-3">
      ...
      {/* Customer Details Modal - No black background */}
      {selectedCustomer && (
            ...
            <div className="p-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span>{getStatusBadge(selectedCustomer.applicationStatus)}</span>
                    </div>
                    <div className="flex justify-between">
              ...
              {selectedCustomer.applicationStatus === "REJECTED" && 
               decisions[selectedCustomer.applicationReferenceNumber] && (
                <div>
                  <h3 className="font-semibold mb-2 text-red-600">Rejection Details</h3>
                  <div className="bg-red-50 p-3 rounded-md text-sm">
                    <p className="font-medium mb-1">Reason:</p>
                    <p>{decisions[selectedCustomer.applicationReferenceNumber].decisionReason}</p>
                    <p className="font-medium mt-2 mb-1">Decision Date:</p>
                    <p>{formatDate(decisions[selectedCustomer.applicationReferenceNumber].decisionDate)}</p>
                    <p className="font-medium mt-2 mb-1">Committee Member:</p>
                    <p>{decisions[selectedCustomer.applicationReferenceNumber].committeeMember}</p>
                  </div>
                </div>
              )}
            </div>
