...
export default function PendingCustomersPage() {
  ...
  const fetchPendingCustomers = async () => {
    try {
      ...
      setError("We're having trouble connecting to the server. Please try again in a moment.");
      setCustomers([]);
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    ...
    <div className="container mx-auto p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      {error && (
        <div className="flex flex-col items-center p-8 bg-white rounded-2xl shadow-lg max-w-2xl mx-auto border-4 border-dashed border-gray-200 text-gray-700">
          ...
          <h2 className="text-3xl font-extrabold text-gray-900 mb-3">You analyzed ALL!</h2>
          <p className="text-lg text-gray-600 text-center mb-6 max-w-md">
           If you want another one to analyze..... <Link className='text-blue-500' href="/dashboard/credit">click here</Link>
          </p>
          <Button
            onClick={fetchPendingCustomers}
      ...
      {!isLoading && customers.length > 0 && (
        <div className="grid grid-cols-1 gap-6">
          {customers.map((customer) => (
            <Card key={customer.id} className="max-w-4xl mx-auto overflow-hidden border border-gray-200 shadow-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="space-y-4 md:col-span-2 mt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label htmlFor={`pestelAnalysisUrl-${customer.id}`} className="text-sm font-medium flex items-center gap-2">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {analysisData[customer.applicationReferenceNumber]?.pestelAnalysisUrl || 'No file uploaded'}
                      </p>
                    </div>

                    <div className="space-y-2">
                      <label htmlFor={`swotAnalysisUrl-${customer.id}`} className="text-sm font-medium flex items-center gap-2">
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        {analysisData[customer.applicationReferenceNumber]?.swotAnalysisUrl || 'No file uploaded'}
                      </p>
                    </div>

                    <div className="space-y-2">
                      <label htmlFor={`riskAssessmentUrl-${customer.id}`} className="text-sm font-medium flex items-center gap-2">
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        {analysisData[customer.applicationReferenceNumber]?.riskAssessmentUrl || 'No file uploaded'}
                      </p>
                    </div>

                    <div className="space-y-2">
                      <label htmlFor={`esgAssessmentUrl-${customer.id}`} className="text-sm font-medium flex items-center gap-2">
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        {analysisData[customer.applicationReferenceNumber]?.esgAssessmentUrl || 'No file uploaded'}
                      </p>
                    </div>

                    <div className="space-y-2">
                      <label htmlFor={`financialNeedUrl-${customer.id}`} className="text-sm font-medium flex items-center gap-2">
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        {analysisData[customer.applicationReferenceNumber]?.financialNeedUrl || 'No file uploaded'}
                      </p>
                    </div>
                  </div>
